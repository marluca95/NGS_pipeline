...
def process_sample(consensus_fastq, output_csv, start_anchor, end_anchor,
    ...
    """Extract codon region between anchors, translate, and write to CSV."""
    results = []
...
def main():
    ...
    for sample_dir in base_dir.glob("GFB-*"):
        ...
        # Select input FASTQ depending on consensus_mode
        if args.consensus_mode == "no_consensus":
            consensus_files = list(sample_dir.glob("*_combined_final.fastq.gz"))
        else:
        ...
        if args.consensus_mode == "no_consensus":
            sample_name = fname.replace("_combined_final.fastq.gz", "")
        else:
        ...
        logger.log(f"Extracting codons for sample {sample_name} ({args.consensus_mode})")
        process_sample(
            consensus_fastq,
            output_csv,
            start_anchor,
